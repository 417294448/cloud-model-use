...
- **speech-recognition 已改为目录页（2026-09-22 确认）**：该页正文现已拆分为子页——`/zh/model-studio/asr-model`（语音识别概述：模型 ID/模式/API/精度增强/情感识别/说话人分离/语言/时长、音频规格、即将下线清单）、`real-time-speech-recognition-user-guide`、`non-realtime-speech-recognition-user-guide`、`improve-asr-accuracy`。ASR 能力信息一律改从 `asr-model` 页取（`python scripts/qwen/fetch_docs.py asr-model`），别再依赖 `speech-recognition` 空壳
- **2026-09-22 新增收录**：`qwen-audio-3.1-asr-flash-streaming`（6元/4.5元每百万 Token）、`qwen-audio-3.1-asr-flash-filetrans`（0.8元/2.7元，支持说话人分离）、`qwen-audio-3.1-asr-flash`（0.8元/2.7元）、`qwen3.8-omni-flash-realtime`（文本/图片/视频入 1.5元、音频入 6元、文本出 4.5元、音频出 12元）、`qwen-audio-3.1-realtime-plus`（文本入 5元/音频入 40元/文本出 40元/音频出 150元）；总览页「语音识别/语音转语音/全模态」推荐位均已切至 3.1 与 3.8 realtime，上代 3.0 行为「上代」
- **未收录待确认（2026-09-22）**：billing 页的 `qwen-audio-3.1-asr-flash-message`（6元/4.5元）与 `qwen-audio-3.1-tts-next`（6元/12元）在分类文档中无能力说明，暂不收录
